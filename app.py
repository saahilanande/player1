...
def login():
    if request.method == "POST":
            if request.form.get("lbutton"):
                ...
                cursor = cnxn.cursor()
                cursor.execute("delete from rng where name = 'p1'")
                cnxn.commit()

                session['username'] = name
                cursor = cnxn.cursor()
                cursor.execute("insert into rng (name,red,green) VALUES('p1','"+red+"','"+green+"');")
                cnxn.commit()
...
def main():
    if request.method == "POST":
            if request.form.get("lbutton"):
                ...
                cursor = cnxn.cursor()
                cursor.execute("UPDATE rng SET "+color+" = '"+amount+"' WHERE name = 'p2';")
                cnxn.commit()
